...
export const all = () => async (c: Context) => {
  const data = await db.select().from(blogTagSchema);
  return c.json(data);
};
...
export const create = () => async (c: Context) => {
  ...
  try {
    ...
    if (error.code === '23505') {
      return c.json({ success: false, message: 'Tag already exists' }, 409);
    }
